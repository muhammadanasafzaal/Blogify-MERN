                ...
                <div className="col-md-12 col-12 mb-4 category-filter">
                    <button className={`btn mr-md-2 ${!selectedCategory ? 'btn-dark' : 'btn-outline'}`} onClick={()=>getBlogs('')}>All Items</button>
                    {
                        categories?.map((item, index) => {
                            return (
                                <button className={`btn mr-md-2 ${selectedCategory == item._id ? 'btn-dark' : 'btn-outline'}`} key={index} onClick={()=>getBlogs(item._id)}>{item.name}</button>
                            )
                        })
                    }
                </div>
                ...
                !blogs?.length ? "No blogs available"
                    ...
                    <div className='row'>
                        ...
                        <div className='col-md-6 col-12 mt-5 mt-md-2'>
                            <div className="row">
                                {
                                    blogs.filter(b => b._id != blogs[0]._id).map((item2, index2) => {
                                        return (
